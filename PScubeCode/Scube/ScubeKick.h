...
public:
    ...
    // Exposes the degree detector's addr value for node v.
    // type=0: out-addr (rows scanned in nodeWeightQuery type 0)
    // type=1: in-addr  (cols scanned in nodeWeightQuery type 1)
    // This value directly controls query latency — the core privacy side-channel.
    addr_type getAddrQuery(string v, int type) {
        ...
        if (type == 0) {
            ...
            return degDetector->addrQuery(key_v, 0);
        } else {
            addr_type addr_v = (hash_v >> fingerprint_length) % width;
            key_type  key_v  = (addr_v << fingerprint_length) + fp_v;
            return degDetector->addrQuery(key_v, 1);
        }
    }
...
bool ScubeKick::kick_out(addr_type& addr_s, fp_type& fp_s, addr_type& addr_d, fp_type& fp_d, w_type& w, addr_type& idx_s, addr_type& idx_d, addr_type* rows, addr_type* cols, int& row_kicks, int& col_kicks)
{
    ...
    for (int kick_num = 0; !inserted; kick_num++)
    {
        ...
        int idx = getMinIndex(kick_index, ROOM * row_kicks * col_kicks);
        if (idx < 0) {
